...
async function runDiag() {
  try {
    ...
    const [profileCount] = await db.execute('SELECT COUNT(*) as count FROM profiles');
    console.log('Total Profiles:', profileCount[0].count);

    const [approvedProfiles] = await db.execute("SELECT COUNT(*) as count FROM profiles WHERE status = 'Approved'");
